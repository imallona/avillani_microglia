...
Mind the iSEE deployments show 'counts', 'CPMs' or 'logCPMs'. My recommendation is to check 'CPMs' or 'logCPMs', so readouts are already normalized/don't depend on sequencing depth.
...
Plots in PNG and SVG format <a href="plots_post/">can be browsed here</a>.
...
tmp2 <- meta[meta$names %in% colnames(tmp$counts),]
rownames(tmp2) <- tmp2$names



tmp$counts <- tmp$counts[, tmp2$names]
tmp$logcpms <- tmp$logcpms[, tmp2$names]

saveRDS(object = SingleCellExperiment(assays = tmp,
